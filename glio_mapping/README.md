...
**GLIO-Mapping** 多传感器融合建图定位系统

GLIO-Mapping 是一个专为长距离动态环境设计的高鲁棒性多传感器融合建图定位系统，已成功应用于2026年人形机器人马拉松竞赛，实现21公里全程自主导航。系统通过GNSS、IMU和雷达点云的紧耦合融合，在复杂户外环境中保持厘米级定位精度。

1. 全赛道验证：完整支持21公里马拉松赛道，通过坡度、障碍物、人群干扰等严苛测试 、多模态融合：GNSS(RTK/PPP)、LiDAR、高精度IMU的紧耦合优化
2. 双重一致性：前端里程计与后端图优化的分层一致性保障

- 软件依赖安装
```shell
sudo apt install libgoogle-glog-dev libgflags-dev
sudo apt-get install gdal-bin libgdal-dev 
sudo apt install ros-noetic-moveit-msgs ros-noetic-serial libpcap-dev
```
...
- 配置说明
```yaml
robosense.yaml 中配置雷达、点云、与imu的话题即可
```
- 数据格式
```yaml
输入话题 
...
​	In this project, the LIO module refers to [FAST-LIO](https://github.com/hku-mars/FAST_LIO) and the pose graph optimization refers to [FAST_LIO_SAM](https://github.com/kahowang/FAST_LIO_SAM) and [LIO_SAM](https://github.com/TixiaoShan/LIO-SAM).The mainly idea is for [FAST-LIO-Multi-Sensor-Fusion](https://github.com/zhh2005757/FAST-LIO-Multi-Sensor-Fusion).Thanks there great work .
...
```
    cd ~/$A_ROS_DIR$/src
    git clone https://github.com/hku-mars/glio_mapping.git
    cd glio_mapping
...
```
    cd ~/$FAST_LIO_ROS_DIR$
    source devel/setup.bash
    roslaunch glio_mapping mapping_avia.launch
...
```
    cd ~/$FAST_LIO_ROS_DIR$
    source devel/setup.bash
    roslaunch glio_mapping mapping_velodyne.launch
```
...
```
roslaunch glio_mapping mapping_marsim.launch
```

### 3.5 PCD file save

Set ``` pcd_save_enable ``` in launchfile to ``` 1 ```. All the scans (in global frame) will be accumulated and saved to the file ``` glio_mapping/PCD/scans.pcd ``` after the FAST-LIO is terminated. ```pcl_viewer scans.pcd``` can visualize the point clouds.
