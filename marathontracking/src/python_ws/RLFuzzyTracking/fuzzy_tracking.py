...
class FuzzyTracking:
    ...
    def compute(self, target_x, yaw_error, curvature):
        t0 = time.time()
        target_x = np.clip(target_x, 0.03, 4.9)
        yaw_error = np.clip(yaw_error, -3.10, 3.10)
        curvature = np.clip(curvature, 0.03, 0.95)
        ...
        self.sim.input["in_theta"] = yaw_error
        self.sim.input["in_curve"] = curvature
        self.sim.compute()
        output = [
...
if __name__ == "__main__":
    ...
    plt.plot(x * 57.3, y_velx)
    plt.plot(x * 57.3, y_velz)
    plt.show()
