...
__device__ void WriteNMSResult(EfficientNMSParameters param, int* __restrict__ numDetectionsOutput,
    ...
    int* __restrict__ nmsIndicesOutput, T threadScore, int threadClass, BoxCorner<T> threadBox, int boxIdxMap,
    int imageIdx, unsigned int resultsCounter)
{
    ...
    nmsIndicesOutput[outputIdx] = boxIdxMap % param.numAnchors;
                ...
                else
                {
                    ...
                    if (write)
                    {
                        ...
                        WriteNMSResult<T>(param, numDetectionsOutput, nmsScoresOutput, nmsClassesOutput, nmsBoxesOutput,
                            nmsIndicesOutput, threadScore[tile], threadClass[tile], threadBox[tile], boxIdxMap[tile],
